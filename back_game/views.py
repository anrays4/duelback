...
def find_match_page(request, table_id):
    ...
    game_room_time_end = GameRoom.objects.all()
...
class FindMatch(APIView):
    ...
    def get_avatar_url(self, user):
        ...
        return '/statics/images/profile-default.jpg'  # مسیر آواتار پیش‌فرض
    ...
    def post(self, request):
        ...
        if WaitingRoom.objects.filter(player_1=my_user).exists() and not (
            ...
            if GameRoom.objects.filter(player_1=my_user, game_room_is_end=False).exists():
                ...
                enemy_avatar = self.get_avatar_url(game_room.player_2)
                ...
                enemy_avatar = self.get_avatar_url(game_room.player_1)
